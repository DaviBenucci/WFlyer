...
export const SCORE_PATH_REVIEW_SEED = STORY_SCORE_SESSION_SEED;
...
export interface ScorePathReviewZone {
  ...
  readonly chapterId: StoryChapterId | null;
  readonly descendingArcLength: number;
  readonly endT: number;
  readonly eventCount: number;
  readonly id: string;
  readonly interactionId: "CARD_SCORE_INTERACTION" | null;
  readonly interactionProfile: ScorePathReviewInteractionProfile | null;
  ...
  readonly projectVisit: ScorePathReviewProjectVisit | null;
  readonly semanticSlotIds: readonly string[];
  readonly startT: number;
  readonly verticalBudget: number | null;
}

export interface ScorePathReviewInteractionProfile {
  readonly cardCount: number;
  readonly expandedFraction: number;
  readonly firstCardStartFraction?: number;
  readonly leadInFraction: number;
  readonly leadOutFraction: number;
  readonly lastCardEndFraction?: number;
  readonly measurementSource: "deterministic-fallback" | "dom-measured";
  readonly nearestLeadInCardWidth: number;
  readonly nearestLeadOutCardWidth: number;
  readonly transformEndFraction?: number;
  readonly transformStartFraction?: number;
}

export interface ScorePathReviewProjectVisit {
  readonly anchor: Vec2;
  readonly cardRect: ScorePathReviewRect;
  readonly measurementSource: "deterministic-fallback" | "dom-measured";
...
export class ReviewCubicSplineScorePath implements ScorePath {
...
export interface AuthoredTrackGeometry {
  readonly chapters: readonly ScorePathReviewChapterLayout[];
  readonly height: number;
...
export function buildAuthoredGeometry(
  candidateId: ScorePathReviewCandidateId,
  mode: ScorePathReviewMode,
  branch: ScorePathReviewBranch,
  compactTrackWidth: number,
...
function composeReviewBranch(branch: ScorePathReviewBranch): ComposedSegment {
  return STORY_SCORE_COMPOSITIONS[branch];
}
...
export function buildZones(
  ...
  for (const [index, range] of geometry.notationRanges.entries()) {
    ...
    if (startT > cursor) {
      ...
      zones.push(
        Object.freeze({
          ...
          id: `connector:${index}`,
          interactionId: range.connectorInteraction ?? null,
          interactionProfile: range.connectorInteractionProfile ?? null,
          kind: "connector",
          startT: cursor,
          endT: startT,
    ...
    const chapterSlotIds =
      range.semanticSlotIds ??
      slotIds.filter((slotId) => slotId.startsWith(`${range.chapterId}:`));
    ...
    zones.push(
      Object.freeze({
        arcLength: polylineArcLength(points),
        barlineAfter: range.barlineAfter ?? true,
        chapterId: range.chapterId,
        descendingArcLength:
          scorePathReviewDescendingPolylineArcLength(points),
        id: `notation:${range.chapterId}:${index + 1}`,
        interactionId: null,
        ...
        points,
        projectVisit: range.projectVisit ?? null,
        verticalBudget: null,
      }),
    );
    cursor = endT;
...
export interface BuildOrganicFlowingModelOptions {
...
export function buildReviewModel(
  ...
  zones: readonly ScorePathReviewZone[],
  composition: ComposedSegment,
  options: BuildOrganicFlowingModelOptions = {},
): ScoreRenderModel {
  ...
  return buildScoreModel({
    ...
    staffSpace,
    staffSampleCount:
      options.staffSampleCount ?? Math.max(1025, path.segmentCount * 25),
    ...
    ...(options.clef === false
      ...
      : {
          clef: {
            t: options.clefT ?? zonePlacementT(origin, 0.07),
          },
        }),
    ...(options.keySignature === false
      ? {}
      : {
          keySignature: {
            fifths: 2,
            t:
              options.keySignatureT ?? zonePlacementT(origin, 0.15),
          },
        }),
    motifs,
    barlines: notationZones
      .filter((zone) => zone !== terminal && zone.barlineAfter)
      .map((zone, index) => ({
        id: `chapter-${index + 1}`,
...
export function scorePathReviewSemanticFingerprint(
  ...
  return storyScoreSemanticFingerprint(composition);
}
